...
create table Direcciones( --Esta tabla es para la direccion del candidato.
id_Direccion int identity,
Calle varchar(50) not null, --Este campo no se puede quedar en blanco.
Num_Int int,
Num_Ext int not null, --Este campo no se puede quedar en blanco.
Colonia varchar(50) not null, --Este campo no se puede quedar en blanco.
CP int not null, --Este campo no se puede quedar en blanco.
Ciudad varchar(50) not null, --Este campo no se puede quedar en blanco.
Provincia varchar(50) not null, --Este campo no se puede quedar en blanco.
Municipio varchar(50) not null, --Este campo no se puede quedar en blanco.
Pais varchar (50) not null default 'México', --Este campo no se puede quedar en blanco. El campo por defecto será México.
...
Nombre varchar(50) not null, --Este campo no se puede quedar en blanco.
Ape_Pat varchar(50) not null, --Este campo no se puede quedar en blanco.
Ape_Mat varchar(50) not null, --Este campo no se puede quedar en blanco.
Sexo varchar(10) not null, --Masculino, Femenino. Este campo no se puede quedar en blanco.
Fecha_Nac date not null, --La fecha de nacimiento será una fecha corta EJEM: 16/04/95. Este campo no se puede quedar en blanco.
Nacionalidad varchar(50),
E_Mail varchar(100),
Estado_Civil varchar(50) not null, --Soltero, Casado, Viudo, Divorciado, etc.
RFC varchar(13), --Solo pueden ser 13 digitos.
No_IMSS varchar(20), --Solo pueden ser 20 digitos.
CURP varchar(18) not null, --Solo pueden ser 18 digitos. Este campo no se puede quedar en blanco.
...
foreign key (id_Direccion) references Direcciones (id_Direccion)--Se hace un foreign key a la tabla Direcciones, ya que el Candidato solo puede tener una sola direccion.
...
--SELECT Pregunta from Preguntas_Test;
--SELECT Respuesta from Respuestas_Test where Pregunta =1;
--SELECT id_Pregunta, Capacidad, Valor from Preguntas_Test;
Go
...
as
BEGIN
	INSERT INTO Candidatos VALUES (@Nombre, @Ape_Pat, @Ape_Mat, @Sexo, @Fecha_Nac, @Nacionalidad, @E_Mail, @Estado_Civil, @RFC, @No_IMSS, @CURP,
...
as
BEGIN
	INSERT INTO Direcciones VALUES (@Calle,@Num_Int,@Num_Ext,@Colonia,@CP,@Ciudad,@Provincia, @Municipio, @Pais);
	--SELECT @IDDireccion = @@IDENTITY;
	UPDATE Candidatos set Candidatos.id_Direccion = @@IDENTITY where id_Candidato = @IDCandidato;
END
...
/****** Object:  StoredProcedure [dbo].[insertararea]    Script Date: 04/12/2015 23:06:32 ******/
...
CREATE TABLE [dbo].[entradas](
 ...
 CONSTRAINT [PK__entradas__167CD61BF853D995] PRIMARY KEY CLUSTERED 
(
	[id_entrada] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]
...
CREATE TABLE [dbo].[salida](
	...
	[descripcion] [varchar](200) NULL,
	[id_empleado] [int] NULL,
 CONSTRAINT [PK__salida__BBF81F2022F27094] PRIMARY KEY CLUSTERED 
(
	[id_salida] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]
